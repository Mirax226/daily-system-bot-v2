...
export async function listRewardsForEdit(userId: string, client: Client = getSupabaseClient()): Promise<RewardRow[]> {
  const { data, error } = await client
    .from(REWARDS_TABLE)
    .select('*')
    .eq('user_id', userId)
  ...
  if (error) {
    console.error({ scope: 'rewards', event: 'list_edit_error', userId, error });
    throw new Error(`Failed to list rewards for edit: ${error.message}`);
  }
...
export async function createReward(
  params: { userId: string; title: string; description?: string | null; xpCost: number },
  client: Client = getSupabaseClient()
): Promise<RewardRow> {
  const payload = {
    user_id: params.userId,
    ...
    xp_cost: params.xpCost,
    is_active: true
  };

  const { data, error } = await client.from(REWARDS_TABLE).insert(payload).select('*').single();
  if (error || !data) {
    console.error({ scope: 'rewards', event: 'create_error', userId: params.userId, payload, error });
    throw new Error(`Failed to create reward: ${error?.message ?? 'unknown error'}`);
  }
  return data as RewardRow;
}

export async function updateReward(
  params: { rewardId: string; patch: Partial<{ title: string; description: string | null; xpCost: number; isActive: boolean }> },
  client: Client = getSupabaseClient()
): Promise<RewardRow> {
  const patchPayload: Record<string, unknown> = {};
  if (params.patch.title !== undefined) patchPayload.title = params.patch.title;
  if (params.patch.description !== undefined) patchPayload.description = params.patch.description;
  if (params.patch.xpCost !== undefined) patchPayload.xp_cost = params.patch.xpCost;
  if (params.patch.isActive !== undefined) patchPayload.is_active = params.patch.isActive;

  const { data, error } = await client.from(REWARDS_TABLE).update(patchPayload).eq('id', params.rewardId).select('*').maybeSingle();
  if (error) {
    console.error({ scope: 'rewards', event: 'update_error', rewardId: params.rewardId, patch: params.patch, error });
...
export async function deleteReward(rewardId: string, client: Client = getSupabaseClient()): Promise<void> {
  const { error } = await client.from(REWARDS_TABLE).delete().eq('id', rewardId);
  if (error) {
    console.error({ scope: 'rewards', event: 'delete_error', rewardId, error });

...
const renderRewardStoreEditorRoot = async (ctx: Context): Promise<void> => {
  ...
  const lines: string[] = [];
  if (!rewards.length) {
    lines.push('No rewards defined yet.', '', 'Add a reward to get started.');
  } else {
    lines.push('Rewards:');
    rewards.forEach((r) => {
      lines.push(`• ${r.title} — ${r.xp_cost} XP${r.is_active ? '' : ' (inactive)'}`);
    });
  }

  const kb = new InlineKeyboard();
  for (const r of rewards) {
    const btn = await makeActionButton(ctx, { label: `${r.title} (${r.xp_cost} XP${r.is_active ? '' : ' — inactive'})`, action: 'rewards.edit_open', data: { rewardId: r.id } });
  ...
  kb.text(addBtn.text, addBtn.callback_data).row().text(backBtn.text, backBtn.callback_data);

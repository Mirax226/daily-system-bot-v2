...
import { logErrorReport, getErrorReportByCode } from './services/errorReports';
...
const buildSettingsKeyboard = async (ctx: Context): Promise<InlineKeyboard> => {
  const backBtn = await makeActionButton(ctx, { label: '⬅️ Back', action: 'nav.dashboard' });
  return new InlineKeyboard().text(backBtn.text, backBtn.callback_data);
};
...
const renderRewardBuyList = async (ctx: Context): Promise<void> => {
  ...
  for (const reward of rewards) {
    const btn = await makeActionButton(ctx, {
      label: `${reward.title} (${reward.xp_cost} XP)`,
      action: 'rewards.confirm',
...
const renderReportcar = async (ctx: Context): Promise<void> => {
  ...
  await renderScreen(ctx, {
    titleKey: 'Reportcar',
    bodyLines: ['Reportcar will be available soon.'],
...
const renderTasks = async (ctx: Context): Promise<void> => {
  const backBtn = await makeActionButton(ctx, { label: '⬅️ Back', action: 'nav.dashboard' });
  await renderScreen(ctx, {
    titleKey: 'Tasks / Routines',
    bodyLines: ['Tasks and routines will be available soon.'],
    inlineKeyboard: new InlineKeyboard().text(backBtn.text, backBtn.callback_data)
  });
};
...
const renderTodo = async (ctx: Context): Promise<void> => {
  ...
  await renderScreen(ctx, {
    titleKey: 'To-Do List',
    bodyLines: ['To-Do List will be available soon.'],
    inlineKeyboard: new InlineKeyboard().text(backBtn.text, backBtn.callback_data)
  });
};
...
const renderPlanning = async (ctx: Context): Promise<void> => {
  ...
  await renderScreen(ctx, {
    titleKey: 'Planning',
    bodyLines: ['Planning will be available soon.'],
    inlineKeyboard: new InlineKeyboard().text(backBtn.text, backBtn.callback_data)
  });
};
...
const renderMyDay = async (ctx: Context): Promise<void> => {
  ...
  await renderScreen(ctx, {
    titleKey: 'My Day',
    bodyLines: ['My Day will be available soon.'],
    inlineKeyboard: new InlineKeyboard().text(backBtn.text, backBtn.callback_data)
  });
};
...
const renderFreeText = async (ctx: Context): Promise<void> => {
  ...
  await renderScreen(ctx, {
    titleKey: 'Free Text',
    bodyLines: ['Free Text capture will be available soon.'],
    inlineKeyboard: new InlineKeyboard().text(backBtn.text, backBtn.callback_data)
  });
};
...
const renderReminders = async (ctx: Context): Promise<void> => {
  ...
  await renderScreen(ctx, {
    titleKey: 'Reminders',
    bodyLines: ['Reminders will be available soon.'],
    inlineKeyboard: new InlineKeyboard().text(backBtn.text, backBtn.callback_data)
  });
};
...
const renderCalendarEvents = async (ctx: Context): Promise<void> => {
  ...
  await renderScreen(ctx, {
    ...
    bodyLines: ['Calendar & Events will be available soon.'],
    inlineKeyboard: new InlineKeyboard().text(backBtn.text, backBtn.callback_data)
  });
};
...
const renderAI = async (ctx: Context): Promise<void> => {
  ...
  await renderScreen(ctx, {
    titleKey: 'AI',
    bodyLines: ['AI features will be available soon.'],
    inlineKeyboard: new InlineKeyboard().text(backBtn.text, backBtn.callback_data)
  });
};
...
bot.hears('⚙️ Settings', async (ctx: Context) => {
  const kb = await buildSettingsKeyboard(ctx);
  await renderScreen(ctx, {
    titleKey: 'Settings',
    bodyLines: ['Choose an option:'],
    inlineKeyboard: kb
  });
});
  ...
  try {
    ...
    switch (action) {
      ...
      case 'rewards.confirm': {
        ...
        const { user: rewardUser } = await ensureUserAndSettings(ctx);
        const enabledReward = telemetryEnabledForUser(rewardUser.settings_json as Record<string, unknown>);
        ...
        await purchaseReward({ userId: rewardUser.id, reward });
        const balance = await getXpBalance(rewardUser.id);
      ...
      case 'dr.templates': {
        ...
        await renderScreen(ctx, {
          ...
          inlineKeyboard: new InlineKeyboard().text(back.text, back.callback_data)
        });
        break;
      }
      ...
      case 'error.send_report': {
        ...
        await ctx.answerCallbackQuery({
          text: 'Error report sent. Thank you.',
          show_alert: true
        });
        await logTelemetryEvent({
